...
def get_glove_embeddings(vocab, dim):#########,x_train):#x_train
##############################################################################
    # old_base_path = ("https://flair.informatik.hu-berlin.de/resources/embeddings/token/")#("https://s3.eu-central-1.amazonaws.com/alan-nlp/resources/embeddings/")
    ...
    _embeddings = np.zeros([len(vocab), 128])
    for word in vocab:
        word_idx = vocab[word]
        doc=nlp(word)
        _embeddings[word_idx] = doc[0].vector

    # print(x_train)
    # print(vocab)
    # _embeddings = np.zeros([len(vocab), 768])
    # # senten=[word for word in vocab]
    # # cau=' '.join(senten)
    # # print("\n\n\n\n\n cau la:", cau)
    # # ### cau=cau + ' .'
    # # doc = phoBERT.extract_features_aligned_to_words(cau)
    # for senten in x_train:
    #     cau=' '.join(senten)
    #     print(cau)
    #     doc = phoBERT.extract_features_aligned_to_words(cau.lower())
    #     print(cau)
    #     for tok in doc:
    #         if (str(tok)!= '<s>')&( str(tok)!='</s>'):
    #             word_idx = vocab[str(tok)]
    #             _embeddings[word_idx] = tok.vector.tolist()

    # # for word in vocab:
    # #     word_idx = vocab[word]

    # #     _embeddings[word_idx] = doc[word_idx+1].vector.tolist()
    return _embeddings
