...
class IndexTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, lower=True, num_norm=True,
        ...
        if initial_vocab:
            # print([initial_vocab])
            self._word_vocab.add_documents([initial_vocab])#2 {'<pad>': 0, 'của': 1, 'ông': 2, 'là': 
            # print(initial_vocab)######{'<pad>': 0, 'của': 1, 'ông': 2, 'là': 3, '.': 4, 'đã': 5, 'người': 6, '...': 7,
            self._char_vocab.add_documents(initial_vocab)

    def label_fit(self, predefined_label):
        self._label_vocab.add_documents(predefined_label) #1 B-LOC
    ...
    def word_fit(self, X):
        # print(X) ##################################### [['Chuyên_môn_hoá', 'là', 'xu_hướng', 'của', 'phát_triển',
